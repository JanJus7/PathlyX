  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-col gap-4 transition-all hover:shadow-md">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          <span className="font-extrabold text-xl text-slate-900">
            {order.price.toFixed(2)} <span className="text-sm text-slate-500 font-medium">PLN</span>
          </span>
          {order.paymentMethod === "cash" && (
             <p className="text-xs text-red-600 font-bold uppercase mt-1">Cash!</p>
          )}
        </div>
      </div>
      ...
      <div className="pt-2 border-t border-slate-100 mt-auto">
        <button className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 px-4 rounded-xl transition-colors active:scale-[0.98]">
